...
REM Compile garbage collector and memory management
...
REM Compile runtime and collections
...
gcc -Wall -Wextra -std=c99 -O2 build/*.o -o build/rubolt-enhanced.exe -lm

if %ERRORLEVEL% EQU 0 (
    echo.
    echo ✓ Enhanced Rubolt interpreter built successfully!
    echo ✓ Executable: build/rubolt-enhanced.exe
    echo.
    echo Testing with nested functions demo...
    build\rubolt-enhanced.exe examples\test_nested_loops.rbo
) else (
    echo.
